...
public:
    ...
    std::string get_output_address() const { std::string result = ""; result = city + " " + street + " " + std::to_string(house) + " " + std::to_string(room);  return result; }
    bool operator>(const adress& other) const {
        return get_output_address() > other.get_output_address();
    }
};
